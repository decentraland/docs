...
See [Code optimization](performance-optimization.md) for more information on how to optimize your scene's code.
...
This however is not always ideal in cases where the meshes might not all be visible at the same time. Don’t merge all static objects together in a single scene, as objects which are outside of the view will still be rendered if they are merged with other visible objects. Try to merge clusters of visible objects together.
 If the meshes are separate, the engine is able to cull the ones that are not visible. See [Meshes on large scenes](../../3d-modeling/meshes.md#Meshes-on-large-scenes) for more details.
...
Texture atlases are a great way to reduce the number of textures in your scene. It allows you to share the same texture across multiple models. See [Shared textures betewen GlTF models](../../3d-modeling/textures.md#Shared-textures-betewen-GlTFs-models) for more details.
...
{% hint style="info" %}
**💡 Tip**: Read more on 3D model best practices in the [3D Modeling Section](../../3d-modeling/3d-models.md)
{% endhint %}
