...
* **¿Vas a crear muchas copias de golpe?** Precarga el modelo primero con el componente `AssetLoad`. Así las copias se crean a partir de un modelo que ya está en memoria, en lugar de que cada una espere la misma descarga. Construir cada copia igual cuesta trabajo, así que una ráfaga grande te puede costar un frame de todos modos: la precarga elimina la espera por el archivo, no el costo de crear las copias.
...
* **Repetir un modelo no agrega texturas ni geometría nuevas.** Veinte postes de luz construidos a partir de un modelo comparten los meshes y las texturas de ese modelo en lugar de sumar al conjunto: el vigésimo no cuesta nada en memoria de texturas. Los materiales son la excepción: el motor crea una instancia de material por objeto renderizado para poder aplicar el clipping de los límites de tu escena, así que la cantidad de materiales sigue a la cantidad de objetos que renderizas, no a la cantidad de modelos que usas. Esas instancias igual comparten las mismas texturas y variantes de shader, así que cuestan un poco de memoria y no tiempo de frame. Lo que acerca una escena al [límite](scene-limitations.md) de texturas es tener muchos modelos *distintos*, cada uno con su propio set — así que consolidar tu escena en torno a una biblioteca más chica de modelos reutilizados ayuda ahí.
