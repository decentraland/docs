...
**You get this for free.** Every UI renderer is positioned inside the device safe area by default, so you don't need to do anything to keep your UI clear of these hardware margins. That default is the `screenInset: 'device'` renderer option — see [Screen inset area](../2d-ui/onscreen-ui.md#screen-inset-area):
