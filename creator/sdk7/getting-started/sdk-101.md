...
{% hint style="info" %}
**💡 Tip**: You can also build scenes by describing what you want to an AI assistant, without writing code by hand. See [Vibe Coding with AI](vibe-coding.md).
{% endhint %}
...
Notice that this file defines a function called `main()`. This function is the entry point to the scene, any code you put there will run when the scene first loads. Lines of code outside this function run earlier, before the scene is fully initialized, so as a rule of thumb write your code inside `main()`. See [Scene lifecycle](coding-scenes.md#scene-lifecycle) for more details.
