...
* **Spawning many copies at once?** Pre-load the model first with the [`AssetLoad` component](pre-load-resources.md). The copies are then created from a model that's already in memory, instead of each one waiting on the same download.
...
* **Repeating a model doesn't introduce new materials or textures.** Twenty lamp posts built from one model reuse that model's materials and textures rather than adding to the set. What pushes a scene against the material and texture [limits](scene-limitations.md) is having many *different* models, each with its own bespoke set — so consolidating your scene around a smaller library of reused models helps there too.
